...
# 测试入口（单文件运行）
# =========================
def run_test_arxiv_api(arxiv_config: Dict[str, Any], db_path: Optional[str] = None) -> None:
    """
    测试 arXiv API 调用逻辑，输出调试信息
    
    Args:
        arxiv_config: 测试用的 arXiv 配置字典
        db_path: 数据库路径；为 None 时使用 DatabaseManager.DEFAULT_DB_PATH
    """
    ...
    print(f"数据库路径: {db_path or DatabaseManager.DEFAULT_DB_PATH}")
...
def main():
    ...
    """
    try:
        from AIpaper2.common_settings import build_arxiv_config
    except ImportError:
        import sys
