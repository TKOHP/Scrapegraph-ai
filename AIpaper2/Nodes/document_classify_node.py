...
class DocumentClassifyNode(BaseNode):
    ...
    def _llm_extract_metadata_and_subjects(self, content: str, subjects: List[str], types: List[str]) -> Dict[str, Any]:
        if getattr(self, "llm_model", None) is None:
            return {}
        options = "、".join(subjects or [])
        type_options = "、".join(types or [])
        tpl = (
            ...
            "- subjects_selected: 从给定主题池中选择的主题数组，只能取自提供的主题池；若无匹配则选择['其他']\n"
            "- paper_type: 从给定类型池中选择论文类型，只能取自类型池且只能选择一个；若无法判断则输出“其他”\n\n"
            "主题池：{options}\n\n"
            "类型池：{type_options}\n\n"
            "{content}"
        )
        ...
        resp = chain.invoke({"options": options, "type_options": type_options, "content": content}) or ""
        try:
            data = json.loads(resp)
            if not isinstance(data, dict):
                return {}
            paper_type = str(data.get("paper_type", "") or "").strip()
            if paper_type not in (types or []):
                paper_type = "其他"

            # 处理主题分类：精确匹配 + 兜底
            raw_subjects = data.get("subjects_selected") or []
            pool = subjects or []
            selected = []
    ...
    def execute(self, state: dict) -> dict:
        ...
        types: List[str] = TYPE_POOL

        llm_enabled = getattr(self, "llm_model", None) is not None
        self.logger.info(
            f"文档分类节点——开始处理 papers={len(papers)} subjects_pool={len(subjects)} types_pool={len(types)} llm_enabled={llm_enabled}"
        )
        ...
        for idx, p in enumerate(papers, start=1):
            try:
                ...
                extract = self._llm_extract_metadata_and_subjects(pdf_text, subjects, types)
                if not extract:
                    skipped += 1
                    updated.append(p)
                    continue
                publish_time = extract.get("publish_time") or self._fallback_publish_time(pdf_text)
                paper_type = extract.get("paper_type") or (p.type or "其他")
                updates = {
                    "meta": json.dumps(
                        {
                ...
                classified += 1
                self.logger.info(
                    f"文档分类节点——第 {idx}/{len(papers)} 篇完成 id={p.id} type={p.type or ''} subjects={p.subject or ''} publishTime={p.publishTime or ''}"
                )
            except Exception as e:
